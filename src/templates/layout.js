		  ...
		  ${SHOW_ADS ? `<script>
		    (function() {
		      ...
		      function adjustAdSlotHeight(slot, ins) {
		        ...
		        if (slot.dataset.adObserved) return;
		        ...
		        var existingIframe = ins.querySelector('iframe');
		        if (existingIframe) {
		          observeIframe(existingIframe);
		          return;
		        }
...
function generateAdSlot(slotIdPc, slotIdMobile, extraClass = '') {
  ...
  <div class="ad-slot ad-slot-section ad-slot--horizontal mobile-only ${extraClass}" style="height:50px;max-height:50px;overflow:hidden">
    <ins class="adsbygoogle" style="display:block;width:100%;height:50px" data-ad-client="ca-pub-9477874183990825" data-ad-slot="${mobileSlot}" data-ad-format="horizontal" data-full-width-responsive="true"></ins>
  </div>`;
}

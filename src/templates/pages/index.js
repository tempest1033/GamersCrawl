  ...
  var insightCardHtml = aiInsight ?
    ...
    if (format === 'vertical') {
	  ...
	  function adSlotMobile(id, extraClass, adSlotId, adFormat) {
	    ...
	    // 가로형: 전체 폭, 높이 100px 기본 (50px 광고 시 자동 축소)
	    if (isHorizontal) {
	    ...
	    // 직사각형: 336x280 고정 크기 + 전체 너비 컨테이너
	    if (isRectangle) {
	      return '<div class="ad-slot ad-slot-section mobile-only ad-slot--rectangle ' + (extraClass || '') + '" id="' + id + '"><ins class="adsbygoogle" style="display:inline-block;width:336px;height:280px" data-ad-client="ca-pub-9477874183990825" data-ad-slot="' + slotId + '"></ins></div>';
	    }
	  ...
	  // 홈페이지 상단 광고 - 전체 너비로 표시 (컨테이너 밖으로)
	  var topAdPc = SHOW_ADS ? '<div class="ad-slot ad-slot-section ad-slot--horizontal pc-only" id="home-top-ad-pc"><ins class="adsbygoogle" style="display:block;width:100%;height:90px" data-ad-client="ca-pub-9477874183990825" data-ad-slot="' + AD_SLOTS.horizontal + '"></ins></div>' : '';

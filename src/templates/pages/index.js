  ...
  var insightCardHtml = aiInsight ?
    ...
    if (format === 'vertical') {
	  ...
	  function adSlotMobile(id, extraClass, adSlotId, adFormat) {
	    ...
	    // 가로형: 전체 폭 반응형, 높이 100px 기본 (50px 광고 시 자동 축소)
	    if (isHorizontal) {
	      return '<div class="ad-slot ad-slot-section mobile-only' + shapeClass + ' ' + (extraClass || '') + '" id="' + id + '"><ins class="adsbygoogle" style="display:block;width:100%;height:100px" data-ad-client="ca-pub-9477874183990825" data-ad-slot="' + slotId + '" data-ad-format="horizontal" data-full-width-responsive="true"></ins></div>';
	    }
	    // 직사각형: 고정 크기 300x250 직접 지정
	    if (isRectangle) {
	      return '<div class="ad-slot ad-slot-section mobile-only' + shapeClass + ' ' + (extraClass || '') + '" id="' + id + '"><ins class="adsbygoogle" style="display:inline-block;width:300px;height:250px" data-ad-client="ca-pub-9477874183990825" data-ad-slot="' + slotId + '"></ins></div>';
	    }
